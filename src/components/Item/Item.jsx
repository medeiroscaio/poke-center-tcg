        ...
        <>
            ...
            <span className={`status ${item.stockStatus}`}>
              {item.stockStatus.replace("-", " ")}
                    ...
                    <td>
                        <button className="edit-btn" onClick={() => handlePopUp(item.id, 1)}>
                            <FiEdit3/>
                        </button>
                        <button className="delete-btn" onClick={() => handlePopUp(item.id, 2)}>
                            <MdDeleteOutline/>
                        </button>
                    </td>
                </tr>
            ))}
            {popView === 1 ? <EditProductPopUp isOpen={isPopUpOpen} onClose={closePopUp} getItems={getItems} updateItem={updateItem} itemID={itemID}/> : ''}
            {popView === 2 ? <DeletePopUp isOpen={isPopUpOpen} onClose={closePopUp} getItems={getItems} deleteItem={deleteItem} itemID={itemID}/> : ''}
        </>
    );
}
